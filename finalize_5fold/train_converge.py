...
def main():
    ...
    t0, step = time.time(), 0
    EVAL_EVERY = float(os.environ.get("MPGAP_EVAL_EVERY", "300"))
    curve_path = os.environ.get("MPGAP_CURVE", "converge_curve.csv")
    cf = open(curve_path, "w")
    print("elapsed_s,step,lr,mae", file=cf, flush=True)
    ...
    while time.time() - t0 < prepare.TIME_BUDGET:
        for g, lat, state, labels in loader:
            ...
            if elapsed >= next_eval:
                mae_pt = prepare.evaluate_mae(predict_fn)
                lr_now = sched.get_last_lr()[0]
                print(f"CURVE {elapsed:.1f} {step} {lr_now:.3e} {mae_pt:.6f}", flush=True)
                print(f"{elapsed:.1f},{step},{lr_now:.3e},{mae_pt:.6f}", file=cf, flush=True)
                model.train()
                next_eval += EVAL_EVERY
